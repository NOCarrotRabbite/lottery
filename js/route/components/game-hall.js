SPA_RESOLVE_INIT = function(transition, sigal) {
    ...
    let title = '';
    let title_common = '<a href="#/index" class="back">\n' +
        '         <svg class="icon" aria-hidden="true">\n' +
        '           <use xlink:href="#icon-fanhui"></use>\n' +
        '         </svg>\n' +
        '       </a>';
    if(type == 1) {
        title = title_common + '<span class="game-hall-title size-36">炫彩大厅</span>';
    }
    if(type == 2) {
        title = title_common + '<span class="game-hall-title size-36">幻彩大厅</span>';
    }
    if(type == 3) {
        title = title_common + '<span class="game-hall-title size-36">瑞彩大厅</span>';
    }
    $('#header').html(title);
